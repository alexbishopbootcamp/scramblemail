  ...
  Mutation: {
    registerUser: async (_, { primaryEmail, password }, context) => {
      try {
        ...
        if(err.code == 11000) { // Duplicate key error
          ...
          throw new ApolloError("An error occurred", 'BAD_USER_INPUT');
        }
      }
    },
    ...
    loginUser: async (_, { primaryEmail, password }, context) => {
      try {
        ...
        if (!user.verified) {
          throw new ApolloError('Please confirm your email address before logging in');
        }
    ...
    refreshToken: async (_, args, context) => {
      try {
        ...
        if (!token) {
          throw new ApolloError('No refresh token found');
        }
    ...
    generateAddress: async (_, args, context) => {
      ...
      console.error('Error generating address:', err);
      throw new ApolloError('Failed to generate unique email address'); // Should only be reached if failsafe triggers
    },
